...
sshpass -p $PASS ssh sw@$IP_END "./end $ITERS $FAULT_PROB" & 
sshpass -p $PASS ssh sw@$IP_MIDDLE "./middle $ITERS $IP_END $FAULT_PROB" &
sshpass -p $PASS ssh sw@$IP_START "./start $ITERS $INPUT $IP_MIDDLE $FAULT_PROB" &
